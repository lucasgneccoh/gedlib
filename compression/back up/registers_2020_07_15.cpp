...
void encode_environment(ged::GEDEnv<UserNodeID, UserNodeLabel, UserEdgeLabel> &env_coded, 
	...
	std::ofstream ofile;
	std::cout<<path + "/" + file_name + ".info_file"<<std::endl;
	ofile.open(path + "/" + file_name + ".info_file", ios::out | ios::binary);
	std::size_t cont=0;
...
void encode_arborescence(
	...
	for(auto const &parent : children){
		for(auto const &child: parent.second){
			...
			if (stdout) {
				std::cout<<parent.first<<" -> "<<child<<std::endl;
				std::cout<<"Node Map"<<std::endl;
				std::cout<<node_map<<std::endl;
				std::cout<<"Compression sets"<<std::endl;
				print_compression_sets(v_d,v_i,v_s,v_is,e_nd,e_ed,e_ni,e_ei,e_s,e_is);
			}

			// Write the file corresponding to the child graph	
			graph_file_name = path + "/" + env_coded.get_graph_name(child) + "_" + std::to_string(child) + ".graph";
			
			if(first){
				ocollection.open(path + "/" + file_name + ".collection", ios::out);	
				first=false;
			}
			else{
				ocollection.open(path + "/" + file_name + ".collection", ios::out | ios::app);
			}
			ocollection<<graph_file_name<<"\n";
			ocollection.close();
			...
			if(g1.num_nodes <= g2.num_nodes){
				...
				for(std::size_t i=0; i<v_i.size(); i++){
					ofile<<"#"<<v_i.at(i)<<"\n";
					for(auto const label : g2.node_labels.at(v_i.at(i))){
						ofile<<"$"<<label.first<<":"<<label.second<<"\n";
					}
					ofile<<"#"<<"end_ins"<<"\n";
				}
				...
				for(std::size_t i=0; i<v_s.size(); i++){
					ofile<<"#"<<v_s.at(i)<<":"<<node_map.image(v_s.at(i))<<"\n";
					for(auto const label : g2.node_labels.at(node_map.image(v_s.at(i)))){
						ofile<<"$"<<label.first<<":"<<label.second<<"\n";
					}
					ofile<<"#"<<"end_subs"<<"\n";
				}
			...
			for(std::size_t i=0; i<e_s.size(); i++){
				ofile<<"#"<<e_s.at(i).first<<","<<e_s.at(i).second<<":"<<node_map.image(v_s.at(i))<<"\n";
				for(auto const label : g2.edge_labels.at(e_s.at(i))){
					ofile<<"$"<<label.first<<":"<<label.second<<"\n";
				}
				ofile<<"#"<<"end_subs"<<"\n";
			}
			...
			for(std::size_t i=0; i<e_ni.size(); i++){
				ofile<<"#"<<e_ni.at(i).first<<","<<e_ni.at(i).second<<"\n";
				for(auto const label : g2.edge_labels.at(e_ni.at(i))){
					ofile<<"$"<<label.first<<":"<<label.second<<"\n";
				}
				ofile<<"#"<<"end_ins"<<"\n";
			}
			for(std::size_t i=0; i<e_ei.size(); i++){
				ofile<<"#"<<e_ei.at(i).first<<","<<e_ei.at(i).second<<"\n";
				for(auto const label : g2.edge_labels.at(e_ei.at(i))){
					ofile<<"$"<<label.first<<":"<<label.second<<"\n";
				}
				ofile<<"#"<<"end_ins"<<"\n";
			}
...
void decode_collection(
	...
	if(in.is_open()){
		while(in>>line){
			...
			switch(type_in){
				case 1: 
					decoded_attributes.at("graphs").at("name").emplace(std::to_string(cont), line);
					cont++;
					continue;
				case 2:
					if(line.at(0)==escape_char){
						attr = line.substr(1);
						decoded_attributes.at("node_attr").emplace(attr, std::map<std::string,std::string> ());
						cont=0;
						continue;
					}
					else{
						decoded_attributes.at("node_attr").at(attr).emplace(std::to_string(cont), line);
						cont++;
						continue;
					}
				case 3:
					if(line.at(0)==escape_char){
						attr = line.substr(1);
						decoded_attributes.at("edge_attr").emplace(attr, std::map<std::string,std::string> ());
						cont=0;
						continue;
					}
					else{
						decoded_attributes.at("edge_attr").at(attr).emplace(std::to_string(cont), line);
						cont++;
						continue;
					}
			}
	...
	*/
	...
	// Get arborescence
	std::vector<size_t> arborescence;
	std::size_t pos = 0;
	std::string parent="";
	std::string collection_file = path + "/" + file_name + ".collection";
	in.open(collection_file);
	...
	if(in.is_open()){
		while(in>>line){
			graph_in.open(line.c_str());
			if(graph_in.is_open()){
				graph_files.emplace_back(line);
				while(graph_in>>line2){
					// First opening will justto get the arborescence
					if(line2.substr(0,5).compare("#from")==0){
						pos = line2.find(":") + 1;
						parent = line2.substr(pos);
						if(parent != "empty"){
							arborescence.emplace_back(std::stoi(parent));
						}
						else{
							arborescence.emplace_back(std::numeric_limits<std::size_t>::max());
						}
						break;
					}
				}
				...
				std::cout<<"Unable to open graph file "<<line<<std::endl;
				exit(1);
			}
		}
		in.close();
		...
		std::cout<<"Unable to open colletion file to decode. Stopping execution"<<std::endl;
		exit(1);
	}

	for(std::size_t i=0; i<arborescence.size(); i++){
		if(arborescence.at(i)==std::numeric_limits<std::size_t>::max()){
			arborescence.at(i)=arborescence.size();
		}
	}

	/*
	std::cout<<"Arborescence after decoding"<<std::endl;
	for(std::size_t i=0; i<arborescence.size(); i++){
	...
	*/
	...
	std::size_t size_v_d=0;
	std::size_t size_v_is=0;
	std::size_t size_v_i=0;
	...
	bool pending_subs=false;
	bool reading_nodes = true;

	std::map<std::size_t, std::size_t> pos_to_id;


	std::map<std::size_t, std::size_t> old_id_to_new;
	std::map<std::string, std::size_t> node_id_to_pos;
	

	std::map<std::size_t, std::size_t> pos_to_node;
	std::map<std::size_t, std::size_t> node_to_pos;
	std::vector<std::size_t> pos_to_save;
	...
	while(!to_do.empty()){
		...
		for(auto const& child : children.at(parent_num)){
			...
			if(graph_in.is_open()){
				...
				//std::cout<<"Graph description:"<<std::endl;
				//describe_graph(g1);
				//std::cout<<"---------------------------------------------------"<<std::endl;
				cont=0;
				cont2=0;
				...
				reading_nodes = true;
				node_id_to_pos.clear();
				
				while(graph_in>>line && line!="#edges" && line!="#end"){
					//std::cout<<"line "<<line<<std::endl;
					switch(cont){
						
						case 0: // reading parent. Already known
							break;
						case 1: // # of vertices
						...
						case 2: // size of v_s
							...
							if(g1.num_nodes > num_nodes){
								size_v_d = g1.num_nodes - num_nodes;
								size_v_is = num_nodes - num_subs;
								if(size_v_d <= size_v_is){
									type_read = 1;									
								}
								else{
									type_read = 2;		
								...
								size_v_i = num_nodes - g1.num_nodes;
								type_read = 3;
							}
							cont2=0;
							break;
						default:
						// Reading data
							//std::cout<<"Type "<<type_read<<std::endl;
							...
							//std::cout<<"Switch type_read "<<line<<std::endl;
							switch(type_read){
								case 1: // delete nodes, read v_d and then substitutions
									
									
									break;
								case 2: // delete nodes, read v_is and then substitutions
									
									break;
								case 3: // insert nodes, read substitutions and then insertions
									if(reading_nodes){
										if(mode=="subs"){
										//if(cont2<num_subs){
											...
											if(line.at(0)==escape_char){
												if(line=="#end_subs"){
													...
													pos_to_save.emplace_back(std::stoi(new_id));
													aux_node_ids.emplace_back(new_id);
													cont2++;
												}
												else{
													// start recording
													...
													if(pos != std::string::npos){
														...
														old_id = line.substr(1, pos-1);
														new_id = line.substr(pos+1);
														std::cout<<"$ subs: " << old_id<<" -> "<<new_id<<std::endl;
														pos_to_node.emplace(cont2, std::stoi(new_id));
														
														label = std::map<std::string, std::string> ();																									
												...
												pos = line.find(":");
												attr_name = line.substr(1, pos-1);
												attr_val = line.substr(pos+1);
												std::cout<<attr_name<<" is "<<attr_val<<std::endl;
												label.emplace(attr_name, attr_val);
											}
										}
										else{
											...
											if(line.at(0)==escape_char){
												if(line=="#end_ins"){
													...
													new_id = line.substr(1);
													std::cout<<"$ insert: " <<new_id<<std::endl;													
													label = std::map<std::string, std::string> ();	
													pos_to_node.emplace(cont2, std::stoi(new_id));
												}
											}
										...
										if(cont2 == size_v_i + num_subs) reading_nodes=false;
									}
									else{
										// Reading edges

										// First change order of nodes
				...
				while(line!="#end"){
					...
					switch(cont){
						...
						// Reading data
							...
							//std::cout<<"Switch type_read "<<line<<std::endl;
							switch(type_read){
								case 0: // insert nodes, read substitutions and then insertions
									
									break;
							
								case 1: // insert nodes, read substitutions and then insertions
				...
				*/
				...
				// Delete the nodes stored in v_d. v_d contains the id, thay may not be the position??

				// Maybe we have to do as in edit_operations
				//std::cout<<"Adding graph "<<std::endl;
				g2.node_labels = aux_node_labels;
				g2.original_node_ids = aux_node_ids;
				g2.num_nodes=0;
				for(std::size_t k=0; k<aux_node_labels.size(); k++){
					g2.num_nodes++;
					g2.node_labels.at(pos_to_node.at(k)) = aux_node_labels.at(k);
					g2.original_node_ids.at(pos_to_node.at(k)) = aux_node_ids.at(k);
	...
	*/
	...
	for(ged::GEDGraph::GraphID i{limits.first}; i<limits.second; i++){
		...
		for(iter = g.edge_list.begin(); iter != g.edge_list.end(); iter++){
			edge = *iter;
			for(auto const label: edge.second){
				edge.second.at(label.first) = decoded_attributes.at("edge_attr").at(label.first).at(label.second);
			}
		}

		env_decoded.load_exchange_graph(g, ged::undefined(), ged::Options::ExchangeGraphType::EDGE_LIST, env.get_graph_name(i)+"_coded", env.get_graph_class(i));
	...
	*/
	...
	if(args.count("encode")>0){
		if(args.at("encode")=="true"){
			...
			encode_arborescence<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel>(encoded_file_path, encoded_file_name,
...
int main(int argc, char* argv[]){
    ...
    while (in_file_collections >> input_collection_file) {
		...
		env = ged::GEDEnv<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel> ();
		decode_collection<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel>(
		input_encoded_file_path,
		input_encoded_file_name,
		env);
