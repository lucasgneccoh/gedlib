...
void describe_graph(ged::ExchangeGraph<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel> exchange_graph){
	std::cout<<"NODES: "<<exchange_graph.node_labels.size()<<std::endl;
	...
	std::cout<<"EDGES: "<<exchange_graph.edge_list.size()<<std::endl;
	for (std::size_t i{0}; i < exchange_graph.num_nodes; i++) {
			...
			*/	
			...
			if(e_ed.size() <= e_is.size()){
				...
				for(auto e : e_ed){
					...
					std::advance (iter,e);
					edge = (*iter);
					from = node_map_aux.image(node_map_id.image(edge.first.first));
					to = node_map_aux.image(node_map_id.image(edge.first.second));
	...
	while(!to_do.empty()){
		...
		for(auto const child : children.at(parent_num)){
			...
			if(graph_in.is_open()){
				...
				for(iter = g1.edge_list.begin(); iter != g1.edge_list.end(); iter++){
					edge = (*iter);
					if(stdout) std::cout<<"Edge: "<<edge.first.first << " - "<< edge.first.second<<std::endl;
					if(stdout) std::cout<<"Edge (aux): "<<node_map_aux.image(edge.first.first) << " - "<< node_map_aux.image(edge.first.second)<<std::endl;
				}
				...
				for (std::size_t i = 0; i < size_read; i++)
				{
					...
					if(stdout) std::cout<<line<<std::endl;
					if(type_read == 0){
						e_ed.emplace_back(std::stoi(line));						
					}
					else{
						e_is.emplace_back(std::stoi(line));
					}
					marker.at(std::stoi(line))=false;
