				...
				if(type_read == 0){
					...
					*/
					...
					for(auto const &m : marker){
						if(m.second){
							if(stdout) std::cout<< "Edge in IS in g1 tilde: "<<m.first.first << " - " << m.first.second<< ". label = " << marker_labels.at(std::make_pair(m.first.first, m.first.second)).at("color") << std::endl;
							...
							aux_edge_labels.emplace_back(marker_labels.at(std::make_pair(m.first.first, m.first.second)));
...
void treat_dataset(std::map<std::string, std::string> args){
	...
	// Write gxl files corresponding to the decompressed versions
	if(args.count("gxl_decompressed")>0 && args.at("gxl_decompressed")=="true"){
		...
		for(std::size_t i=graph_ids.first; i<graph_ids.second; i++){
			...
			env_decoded.save_as_gxl_graph(i,  args.at("encoded_file_path") + "/decoded/" +env_decoded.get_graph_name(i));
			
		}	
		env_decoded.save_graph_collection(args.at("encoded_file_path") + "/decoded/" + args.at("encoded_file_name") + ".xml",  gxl_file_names,  graph_classes);
	}
}
...
int main(int argc, char* argv[]){
	...
	if(argc>16) input_gxl_decompressed = argv[16];
	...
	args.emplace(std::make_pair("gxl_decompressed",input_gxl_decompressed));
