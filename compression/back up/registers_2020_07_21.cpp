		...
		if(node_map.image(edge.first.first)==ged::GEDGraph::dummy_node() || node_map.image(edge.first.second)==ged::GEDGraph::dummy_node()){
			...
			if(g2.adj_matrix[node_map.image(edge.first.first)][node_map.image(edge.first.second)]==0){
				...
				if(compare_label(edge.second,g2.edge_labels.at(std::make_pair(node_map.image(edge.first.first),node_map.image(edge.first.second))))){
					e_is.emplace_back(index);
				}
				else{
					e_s.emplace_back(index);
...
void encode_arborescence(
	...
	for(ged::GEDGraph::GraphID i{limits.first}; i<limits.second; i++){
		if(i==root) continue;
		graph_file_name = path + "/" + env_coded.get_graph_name(i) + "_" + std::to_string(i) + ".graph";
	...
	for(auto const &parent : children){
		//if(stdout) std::cout<<"Parent: "<<parent.first<<", # of children: "<<parent.second.size()<<std::endl;
		if (parent.second.size()==0) continue;
		//if(stdout) std::cout<<"Going into for"<<std::endl;
		for(auto const &child: parent.second){
			...
			//if(stdout) std::cout<<"Get compression sets"<<std::endl;

			// Sort edge list to ensure that indexes are well preserved
			g1.edge_list.sort(compare_edges<UserEdgeLabel>);
			...
			node_map_aux.clear(),
			node_map_aux = get_aux_node_map(g1.num_nodes, g2.num_nodes, v_d, v_i);
			...
			*/	
			...
			//if(stdout) std::cout<<"REC E"<<std::endl;
			...
			for(std::size_t i=0; i<e_s.size(); i++){
				ofile<<escape_char<<e_s.at(i)<<separator;
			...
			for(std::size_t i=0; i<e_ni.size(); i++){
				...
				edge = (*iter);
				if(node_map.pre_image(edge.first.first) != ged::GEDGraph::dummy_node()){
					from = node_map_aux.image(node_map.pre_image(edge.first.first));
				}
				else{
					from = v_i_map.at(edge.first.first);
				}
				if(node_map.pre_image(edge.first.second) != ged::GEDGraph::dummy_node()){
					to = node_map_aux.image(node_map.pre_image(edge.first.second));
				}
				else{
...
void decode_collection(
	...
	*/
	...
	std::vector<std::size_t> v_all_but_d;
	...
	while(!to_do.empty()){
		...
		for(auto const child : children.at(parent_num)){
			...
			if(graph_in.is_open()){
				...
				switch(type_read){
					...
					case 2:
						...
						for (std::size_t i = 0; i < num_nodes - num_subs; i++)
						{
							...
							v_all_but_d.emplace_back(std::stoi(line));
					...
					case 3:
						...
						if(stdout) std::cout<<"Insertions"<<std::endl;
						...
						for (std::size_t i = 0; i < num_nodes - g1.num_nodes; i++)
						{
							...
							label.clear();
							graph_in >> line;
							while(line.at(0) != escape_char && ! graph_in.eof() && cont2<num_node_attr){
								pos = line.find(":");
				...
				// Read substitutions
				if(stdout) std::cout<<"Substitutions"<<std::endl;
				if(type_read!=3 && num_subs>0) graph_in >> line; // line with escape char already read in the insertion part
				for (std::size_t i = 0; i < num_subs; i++)
				{
					...
					v_all_but_d.emplace_back(std::stoi(second));
					map_indices.emplace(std::stoi(second), aux_node_ids.size());
					aux_node_ids.emplace_back(second);
					...
					graph_in >> line;
					if(stdout) std::cout<<line<<std::endl;
					// TODO Change limits using num_node_attr and num_edge_attr
					while(line.at(0) != escape_char && ! graph_in.eof() && cont2<num_edge_attr){
				...
				if(type_read!=3){ // V > V'
					if(type_read==2){
						for(std::size_t n = 0; n<g1.num_nodes; n++){
							if(std::find(v_all_but_d.begin(), v_all_but_d.end(), n) == v_d.end()){
								v_d.emplace_back(n);
							}
						}
					}
				...
				g2.node_labels = std::vector<UserNodeLabel>(aux_node_labels);
				g2.original_node_ids = std::vector<UserNodeID>(aux_node_ids);
				...
				// -------------------------------------------------------------------
				// E
				if(stdout) std::cout<<"_edges"<<std::endl;
				marker = std::vector<bool> (g1.num_edges, true);

				// Line 1: # edges
				if(type_read!=3 && num_subs==0) graph_in >> line;
				if(stdout) std::cout<<line<<std::endl;
				...
				for (std::size_t i = 0; i < size_read; i++)
				{
					...
					if(stdout) std::cout<<line<<std::endl;
					if(type_read == 0){
						e_ed.emplace_back(std::stoi(line.substr(1)));						
					}
					else{
						e_is.emplace_back(std::stoi(line.substr(1)));
					}
					marker.at(std::stoi(line.substr(1)))=false;
				...
				{					
					...
					while(graph_in >> line && line.at(0) != escape_char){
						pos = line.find(":");
						first = line.substr(0,pos);
						second = line.substr(pos+1);
				...
				// Read insertions
				if(stdout) std::cout<<"Insertions"<<std::endl;
				if(num_edges - num_subs - e_is.size() >0 && num_subs == 0) graph_in >> line;
...
get_compression_data(
	...
	if(args.count("encode")>0){
		if(args.at("encode")=="true"){
			...
			env_coded, encoded_attributes, encoded_file_path, encoded_file_name, env, alphabets, dummy, empty_id);
			...
			if(args.at("ged_refine_method") == "branch_fast"){
				...
				if(args.at("ged_refine_method") == "branch_tight"){
					...
					if(args.at("ged_refine_method") == "ipfp"){
						env.set_method(ged::Options::GEDMethod::IPFP, "--threads " + ged_refine_method_options + " --initialization-method BRANCH_UNIFORM");
					}
					else{
	...
	if(args.count("encode")>0){
		if(args.at("encode")=="true"){
			...
			encode_environment<ged::GXLNodeID, ged::GXLLabel, ged::GXLLabel>(
			env_dummy, encoded_attributes, encoded_file_path, encoded_file_name, env_uncoded, alphabets, arborescence_ref, empty_id);
...
int main(int argc, char* argv[]){
	...
	if(argc>15) input_encode = argv[15];
	...
	args.emplace(std::make_pair("encode",input_encode));
    ...
    while (in_file_collections >> input_collection_file) {
		...
		if(args.count("encode")>0 && args.at("encode")=="true"){
			...
			std::cout<<"############   Collection AFTER encoding   ################"<<std::endl;
			...
			std::cout<<"############   Collection AFTER encoding - end   ################"<<std::endl;
