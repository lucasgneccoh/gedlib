...
if __name__ == "__main__":
	...
	"""
	# To test the make_blobs functions
	
	import matplotlib.pyplot as plt
	# Plot center and median
	median_file = "/home/lucas/Documents/stage_gedlibpy/stage/cpp/data/output/median.gxl"
	center_file = "/home/lucas/Documents/stage_gedlibpy/stage/cpp/data/output/center.gxl"
	
	median, _ = loadGXL(median_file)
	center, _ = loadGXL(center_file)
	fig, ax = plt.subplots(1,2, figsize=(14,6))
	draw_Letter(median, ax[0])
	draw_Letter(center, ax[1])	
	ax[0].set_title("MEDIAN")
	ax[1].set_title("CENTER")
	plt.show()
	
	
	# plot some random graphs from the blob
	blob_file = "/home/lucas/Documents/stage_gedlibpy/stage/cpp/data/output/blob_samples.xml"
	datapath = "/home/lucas/Documents/stage_gedlibpy/stage/cpp/data/output/"
	data, y, struct, labels = loadFromXML(blob_file, dataset_path=datapath)
	nPlots = min(4, len(data))
	fig, ax = plt.subplots(1,nPlots, figsize=(14,6))
	for i in range(nPlots):
		draw_Letter(data[i], ax[i])
	plt.show()
	"""
	
	# Plot 2 graphs
	import matplotlib.pyplot as plt
	filename = "/home/lucas/Documents/stage/gedlib/compression/data/collections/Letter_mixed.xml"
	
	dataset_path = "/home/lucas/Documents/stage/gedlib/data/datasets/Letter/LOW"
	
	data, y, struct, labels = loadFromXML(filename,dataset_path) 

	
	fig, ax = plt.subplots(1,len(data), figsize=(14,6))
	for i, g in enumerate(data):
		draw_Letter(g, ax[i])
		ax[i].set_title(g.name)
	
	plt.show()
	
	
	filename = "/home/lucas/Documents/stage/gedlib/compression/data/output/encoded/Letter_test/decoded/Letter_mixed.xml"
	
	data, y, struct, labels = loadFromXML(filename) 
